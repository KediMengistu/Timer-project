...
export class TimersUtility {
  ...
  guestCompleteCreateTimer(createTimerDTO: CreateTimerDTO): GuestTimer {
    ...
    if(numberOfBreaks % 2 !== 0){
      ...
      for(let i: number = 1; i <= numberOfBreaks; i++) {
        let startTimeForBreakI: Date = new Date(startTimeMS + (i * workDurationMS) + ((i - 1) * breakDurationMS));
        let endTimeForBreakI: Date = new Date(startTimeForBreakI.getTime() + breakDurationMS);
        let guestTimerBreak: GuestTimerBreak = this.guestCompleteCreateTimerBreakIHelper(startTimeForBreakI, endTimeForBreakI, i, guestTimer);
  ...
  private guestCompleteCreateTimerBreakIHelper(startTimeForBreakI: Date, endTimeForBreakI: Date, i: number, guestTimer: GuestTimer): GuestTimerBreak {
    ...
    const guestTimerBreak: GuestTimerBreak = new GuestTimerBreak(guestTimerBreakObject);
  ...
  guestPausePlayTimerSettingsConfiguration(guestTimer: GuestTimer):     
    ...
    const totalMs = guestTimer.getUnpausedTime().getTime() - guestTimer.getPauseTime().getTime();
    let pausedDurationInMs: number = guestTimer.getPausedDurationInMs();
    pausedDurationInMs !== undefined ? pausedDurationInMs += totalMs : pausedDurationInMs = totalMs;
    const delayedEndTime = new Date(guestTimer.getEndTime().getTime() + totalMs);
    return {
      delayedEndTime,
      pausedDurationInMs
    }
